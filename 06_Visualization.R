...
# ------------------------------------------ #
# Do the models for overall trajectory / disturbance
# ------------------------------------------ #
# (0) both segments significant (or no break and significant), 
# (1) only first segment significant, (2) only 2nd segment significant, 
# (3) both segments insignificant (or no break and not significant)
# Do S I D (A no Dist) and S I D (B no dist)
#  -> SR <- 
source("0005_AveragedPrediction.R")
o <- out %>% dplyr::filter(Abrupt %in% c(NA,"TC") )
# subset to studies with stable comparison available
o <- subset(o, SS %in% names(which(rowSums(table(o$SS,o$Break_binom)>0) == 2)) )
# --- #

# # Stable first
# o1 <- o %>% dplyr::filter(Break_binom == 0) # table(o$flag_type,o$ComparisonGroup)
# # Disturbed sites
# o2a <- o %>% dplyr::filter(Break_binom == 1) %>% dplyr::filter(flag_significance == 3) %>% 
#   mutate( CompNew = "A-D" ) # Ref - Stable disturbed 
# o2b <- o %>% dplyr::filter(Break_binom == 1) %>% dplyr::filter(!flag_significance %in% c(1,3) )
# # Greening and Browning - Disturbed
# o2b$CompNew <- NA
# o2b$CompNew[which(o2b$Trajectory=="I")] <- "B-D"
# o2b$CompNew[which(o2b$Trajectory=="D")] <- "C-D"
# o2 <- rbind(o2a,o2b) %>% mutate(CompNew = factor(CompNew, levels = c("A-D","B-D","C-D")))
# o <- rbind(o1 %>% rename(CompNew = ComparisonGroup),o2 %>% dplyr::select(-ComparisonGroup));rm(o1,o2,o2a,o2b)

#sfit.overall <- glmer(Species_richness ~ CompNew + (1|SS) +(1|LCLU)+ (1|SSB),data=o,family = "poisson")
#sjPlot::sjp.glmer(sfit.overall,type = "fe",prnt.plot=T,fade.ns=T,show.intercept = F)
#sfit.overall.x <- format.results(sfit.overall,"CompNew") 

#o <- out[-which(out$Trajectory == "S" & out$flag_significance == 0 ),] 
#o <- out[-which(o$Trajectory %in% c("I","D") & o$flag_significance %in% c(1,3)  ),]
#o <- o[which(o$flag_significance %in% c(0,2) ),]# Also only those where both or sec. segment is sig.
#o <- o[-which(o$ComparisonGroup == "A" ) , ] #Remove sites with stable trend before trend
sfit.overall <- glmer(Species_richness ~ BinTrend + (1|SS)+(1|LCLU)+(1|SSB)+(1|SSBS),data=o,family = "poisson")
checkConv(sfit.overall)
sjPlot::sjp.glmer(sfit.overall,type = "fe",prnt.plot=T,fade.ns=T,show.intercept = F)
sfit.overall.x <- format.results(sfit.overall,"BinTrend",maxlevels = 6) 

# Rarefied richness
#rfit.overall <- glmer(Richness_rarefied ~ BinTrend + (1|SS)+(1|LCLU) + (1|SSB),data=o,family = "poisson",na.action = na.exclude)
#checkConv(rfit.overall)
#sjPlot::sjp.glmer(rfit.overall,type = "fe",prnt.plot=T,fade.ns=T,show.intercept = F)
#rfit.overall.x <- format.results(rfit.overall,"BinTrend",maxlevels = 6) 
# Now with breaks, but compare to stable non-disturbed global only
#o <- out[-which(out$Trajectory == "S" & out$flag_significance == 0 ),] 
#o <- o[-which(o$Trajectory %in% c("I","D") & o$flag_significance %in% c(1,3)  ),]
# o <- out[which(out$flag_significance %in% c(0,2) ),]# Also only those where both or sec. segment is sig.
# o <- o[-which(o$ComparisonGroup == "A" ) , ] #Remove sites with stable trend
# rfit.overall <- glmer(Richness_rarefied ~ Trajectory + (1|SS)+(1|LCLU) + (1|SSB),data=o,family = "poisson")
# checkConv(rfit.overall)
# sjPlot::sjp.glmer(rfit.overall,type = "fe",prnt.plot=T,fade.ns=T,show.intercept = F)
# rfit.overall.x2 <- format.results(rfit.overall,"Trajectory") %>% mutate(model = "Disturbed")
# rfit.overall.x2 <- rfit.overall.x2[-which(rfit.overall.x2$term == "(Intercept)"),] # Exclude intercept as the same as above
# rfit.overall.x <- rbind(rfit.overall.x1,rfit.overall.x2) 

# Abundance overall 
afit.overall <- glmer(logabund ~ BinTrend + (1|SS)+(1|LCLU) + (1|SSB),data=o,family = "gaussian",na.action = na.exclude)
checkConv(afit.overall)
sjPlot::sjp.glmer(afit.overall,type = "fe",prnt.plot=T,fade.ns=T,show.intercept = F)
afit.overall.x <- format.results(afit.overall,"BinTrend",maxlevels = 6)
# Now with breaks, but compare to stable non-disturbed global only
#o <- out[-which(out$Trajectory == "S" & out$flag_significance == 0 ),] 
...
# o <- out[which(out$flag_significance %in% c(0,2) ),]# Also only those where both or sec. segment is sig.
# o <- o[-which(o$ComparisonGroup == "A" ) , ] #Remove sites with stable trend
# afit.overall <- glmer(logabund ~ Trajectory + (1|SS)+(1|LCLU) + (1|SSB),data=o,family = "gaussian")
# checkConv(afit.overall)
# sjPlot::sjp.glmer(afit.overall,type = "fe",prnt.plot=T,fade.ns=T,show.intercept = F)
# afit.overall.x2 <- format.results(afit.overall,"Trajectory") %>% mutate(model = "Disturbed")
# afit.overall.x2 <- afit.overall.x2[-which(afit.overall.x2$term == "(Intercept)"),] # Exclude intercept as the same as above
# afit.overall.x <- rbind(afit.overall.x1,afit.overall.x2) 

# And finally the PIE
piefit.overall <- glmer(asPIE ~  BinTrend + (1|SS)+(1|LCLU) + (1|SSB),data=o,family = "gaussian",na.action = na.exclude)
checkConv(piefit.overall)
sjPlot::sjp.glmer(piefit.overall,type = "fe",prnt.plot=T,fade.ns=T,show.intercept = F)
piefit.overall.x <- format.results(piefit.overall,"BinTrend",maxlevels = 6)

# --- #  # Make Figure
# Get binary comparison
full <- rbind(sfit.overall.x %>% mutate(type="Species richness"),
              afit.overall.x %>% mutate(type = "Total abundance"),
#              rfit.overall.x %>% mutate(type = "Rarefied richness"),
              piefit.overall.x %>% mutate(type= "Probability of interspecific encounter") )
#full$Comparison <- str_split(full$term,"CompNew",simplify = T)[,2]
full$Comparison <- str_split(full$term,"BinTrend",simplify = T)[,2]
full$Comparison[which(full$Comparison=="")] <- "ND"
full$type <- factor(full$type,levels=c("Species richness","Total abundance","Rarefied richness","Probability of interspecific encounter"),labels = c("Species richness","Total abundance","Rarefied richness","Probability\n of interspecific encounter") )

#full$Comparison <- factor(full$Comparison, levels = c("S","B","C","A-D","B-D","C-D"), labels = c("S","MG","MB","SD","G","B") )
full$Comparison <- factor(full$Comparison, levels = c("Largest NTC","Large NTC","Small NTC","ND","Small PTC","Large PTC","Largest PTC"), labels = c("<<<","<<","<","ND",">",">>",">>>"))

brownGreen <- c("brown","#8B4513", "#CD853F","grey20","#7CFC00","#21B200","#4B8B3C")

# Correct for axis
full[,c("estimate","se.low","se.high")] <- full[,c("estimate","se.low","se.high")] -1 
#full$p.string2 <- gsub('[[:digit:]]+', '', full$p.string); full$p.string2 <- gsub("\\.","",full$p.string2); full$p.string2 <- gsub("^[[:space:]]+|[[:space:]]+$", "", full$p.string2)
full$p.stars[which(full$Comparison=="ND")] <- ""

g3 <- full  %>%  
  dplyr::filter(type !=  "Rarefied richness" ) %>% 
  ggplot(.,aes(x=Comparison,y=estimate, ymin=se.low, ymax=se.high,color=Comparison) ) + 
  theme_tufte(base_size = 20,base_family="Gill Sans MT",ticks = T) +
  theme(panel.grid.major = element_blank(),panel.grid.minor=element_blank() ) +
  # Point range
  geom_pointrange(fatten = 3,size=1.5) + 
  geom_hline(yintercept = 0,linetype="dotted") +
  #geom_vline(xintercept = 4.5,linetype = "dotted") +
  scale_y_continuous(breaks=pretty_breaks(4)) +
  scale_color_manual(values = brownGreen ) + 
  # Add significance symbols
  geom_text(aes(x = Comparison, y=se.low-.02, label = p.stars ), size = 4,colour = "black") +
  # Add text
  #geom_text(aes(label=nSSBS, y=max(estimate) + 0.015*max(estimate)),colour="grey20", size=3,angle=90) +
  guides(shape="none",color="none") +
  #scale_shape_manual(values = c(16,15)) +
  facet_wrap(~type,scales = "fixed",ncol = 1) +
  labs(x="",y="Measure difference (% \u00B1 1 SE)") +
  theme(axis.text.x = element_text(size=19),axis.ticks.x = element_blank()) +
  theme(panel.spacing.x =  unit(1, "lines"))
g3  
ggsave("newFigures/F1Overall_3.png",plot = g3,width = 6,height = 8,dpi = 400)

# ---- #
library(cowplot)
pg <- plot_grid(g1,g3+labs(y=""),align = "h",axis = "lb",labels = "AUTO",hjust = -1)
cowplot::ggsave("newFigures/F1Overall_1-3COMB.png",plot = pg,width = 10,height = 8,dpi = 400)
# --- #
